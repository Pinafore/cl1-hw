...
class TestSequenceFunctions(unittest.TestCase):
    ...
    def test_censor(self):
        ...
        self.assertEqual(censored_a, censored_b)
    ...
    def test_lm(self):
        ...
        # Test Add one
        self.assertEqual(self.lm.mle("<s>", "b"), log(1.0 / 5.0))
        self.assertEqual(self.lm.mle("<s>", "a"), log(2.0 / 5.0))
        self.assertEqual(self.lm.mle("a", "a"), log(2.0 / 6.0))
        self.assertEqual(self.lm.mle("a", "b"), log(2.0 / 6.0))
        self.assertEqual(self.lm.mle("a", "c"), log(2.0 / 6.0))

        # Test Jelinek Mercer (assuming .5 interpolation
        self.assertEqual(self.lm.mle("<s>", "b"), log((0.0 + 1.0 / 5.0) / 2.0))
        self.assertEqual(self.lm.mle("<s>", "a"), log((1.0 + 2.0 / 5.0) / 2.0))
        self.assertEqual(self.lm.mle("a", "a"), log((0.5 + 2.0 / 5.0) / 2.0))
        self.assertEqual(self.lm.mle("a", "b"), log((0.5 + 1.0 / 5.0) / 2.0))
        self.assertEqual(self.lm.mle("a", "c"), log((0.5 + 1.0 / 5.0) / 2.0))
